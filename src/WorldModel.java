...
public final class WorldModel {
    ...
    */
    ...
    private void parseYoshi(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.YOSHI_NUM_PROPERTIES) {
            Entity entity = Entity.createYoshi(id, pt, Double.parseDouble(properties[Entity.YOSHI_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.YOSHI_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Entity.YOSHI_KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.YOSHI_KEY, Entity.YOSHI_NUM_PROPERTIES));
        }
    }
    ...
    private void parseStump(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.STUMP_NUM_PROPERTIES) {
            ...
            this.tryAddEntity(entity);
            this.background[0][1] = new Background("grass", imageStore.getImageList(Entity.STUMP_KEY));
    ...
    public void setBackground(Point p, ImageStore imageStore){
        this.background[p.y][p.x] = new Background("grass", imageStore.getImageList("grass"));
    ...
    //is only called when changing background with click
    //will shift the point if it is out of bounds so we don't run into any erros
    public Point shiftPoint(Point pos){
        int dx = 0, dy = 0;
        if(pos.x + 2 >= this.numCols || pos.x - 2 < 0){ //if we click a point and 2 points above or below are out of bounds, fix it
            dx = (pos.x - 2 < 0)? 0 - (pos.x - 2): this.numCols - (pos.x + 2);
            System.out.println("New Pointer: " + (0 -(pos.x - 2)));
        }

        if(pos.y + 2 >= this.numRows || pos.y - 2 < 0){
            dy = (pos.y - 2 < 0)? 0 - (pos.y-2): this.numRows - (pos.y + 2);
        }
